...
// clang-format off
                ...
                if (kernel[k_rows][k_cols]) {
                    if (src_buf[k_rows][k_cols].range(k + (depth - 1), k) < min) {
                        min = src_buf[k_rows][k_cols].range(k + (depth - 1), k);
                    }
                }
            }
        }
        packed_val.range(k + (depth - 1), k) = min;
    }
...
// clang-format off
        ...
        if (col < (img_width >> XF_BITSHIFT(NPC))) {
            if (row < img_height) {
                ...
                buf[row_ind[0]][col] = value[col]; // Read data
                rd_ind++;
            } else {
...
// clang-format off
			...
			src_buf[extract_px][K_COLS-2]=OutputValue;
			for(int extract_py=0;extract_py<K_COLS-2;extract_py++)
			{
        ...
        XF_PTUNAME(TYPE) OutputValues[XF_NPIXPERCYCLE(NPC)];
            ...
            erode_function_apply<PLANES, TYPE, K_ROWS, K_COLS>(&OutputValue, src_buf_temp_med_apply,
                                                                kernel); // processing 8-pixels

         value[col]=OutputValue;
...
// clang-format off
    ...
    for(ap_uint<13> i_col=0;i_col<img_width>>XF_BITSHIFT(NPC);i_col++)
    {
		...
		#pragma HLS UNROLL
    	// clang-format on
    	value[i_col] = 0; // buf[K_ROWS>>1][i_col];
    }
...
// clang-format off
        ...
        Process_function_d<ROWS, COLS, PLANES, TYPE, NPC, WORDWIDTH, TC, K_ROWS, K_COLS>(
            _src, kernel, _dst, buf, src_buf, value,P0, img_width, img_height, shift_x, row_ind, row, rd_ind, wr_ind);
